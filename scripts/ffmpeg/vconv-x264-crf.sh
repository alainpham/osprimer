...
echo "ffmpeg -y -i $1 -preset slow -pix_fmt yuv420p -c:v libx264 -crf ${crf} -c:a aac -b:a ${ab}k -f ${format} ${filename}-x264-crf${crf}k-aac-ab${ab}k.${format}"

ffmpeg -y -i $1 -preset slow -pix_fmt yuv420p -c:v libx264 -crf ${crf} -c:a aac -b:a ${ab}k -f ${format} ${filename}-x264-crf${crf}k-aac-ab${ab}k.${format}
